...
def copy_nodetree_callback(self, context):
    if context.space_data.tree_type != "an_AnimationNodeTree": return
    self.layout.operator("an.copy_node_tree", text="Copy", icon="COPYDOWN")

def register():
    bpy.types.NODE_MT_editor_menus.append(subprograms_menu_callback)
    bpy.types.NODE_HT_header.append(copy_nodetree_callback)
    bpy.types.NODE_HT_header.append(remove_nodetree_callback)

def unregister():
    bpy.types.NODE_MT_editor_menus.remove(subprograms_menu_callback)
    bpy.types.NODE_HT_header.remove(copy_nodetree_callback)

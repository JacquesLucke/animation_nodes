...
class MarchingSquaresNode(bpy.types.Node, AnimationNode):
    ...
    def create(self):
        ...
        self.newInput("Integer", "Z Divisions", "zDivisions", value = 1, minValue = 1)
    ...
    def execute(self, xDivisions, yDivisions, zDivisions, size, index, falloff):
        cdef double xDis, yDis
        cdef Vector3DList points
        points, xDis, yDis = getGridPoints(xDivisions, yDivisions, zDivisions, size, size, size)

        cdef FloatList strengths = self.getFieldStrengths(points, falloff)

        cdef int nx, ny, amountCell
        nx, ny = limitAmount(xDivisions), limitAmount(yDivisions)
        amountCell = (nx - 1) * (ny - 1)

        cdef Vector3DList _points = Vector3DList()
        cdef float a, b, c, d
        cdef Py_ssize_t i, j, k, ia, ib, ic, id
        ...
        for i in range(ny - 1):
            ...
            for j in range(nx - 1):
                ...
                a, b, c, d = strengths[ia], strengths[ib], strengths[ic], strengths[id]
                meshes.append(getMesh(points, _xDis, _yDis, ia, ib, ic, id, a, b, c, d))
                k += 1

        return Mesh.join(*meshes), _points
...
def getMesh(Vector3DList points, double _xDis, double _yDis, ia, ib, ic, id, float a, float b, float c, float d):
    cdef int index = binaryToDecimal(a, b, c, d)
    ...
    if index == 0:
        ...
        edges.append((2,3))
        vertices.append(points[id])
        vertices.append(points[ia])
...
cdef getGridPoints(xDivisions, yDivisions, zDivisions, size1, size2, size3, distanceMode = "SIZE"):
    cdef:

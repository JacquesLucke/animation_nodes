...
class ChangeMatrixPivotNode(bpy.types.Node, AnimationNode):
    ...
    def getExecutionCode(self):
        ...
        if pivot == "AXES_XXZ":
            yield "matrixRotation = animation_nodes.algorithms.rotations.directionToMatrix(normal, end - start, 'Z', 'X')"
            yield "pivotMatrix = Matrix.Translation(start) * matrixRotation.normalized()"
        if pivot == "AXES_XXZZ":
            yield "matrixRotation = (animation_nodes.algorithms.rotations.directionToMatrix(endZ - startZ, endX - startX, 'Z', 'X')).normalized()"

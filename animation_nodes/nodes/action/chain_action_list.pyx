...
class ChainActionListNode(bpy.types.Node, AnimationNode):
    bl_idname = "an_ChainActionListNode"
    bl_label = "Chain Action List"
    ...
    def create(self):
        self.newInput("Action List", "Action list", "list")
    ...
    def execute(self, list):
        ...
        if not isinstance(r, BoundedAction):
            self.raiseErrorMessage("only bounded actions can be chained")

        i = 1
        while i < length:
            b = list[i]
            if isinstance(b, BoundedAction):
                r = ChainAction(r, b, self.relative)
            i += 1

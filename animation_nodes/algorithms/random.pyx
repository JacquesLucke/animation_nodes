...
def getUniformRandom(seed, min, max):
    return uniformRandomDouble(seed, min, max)
...
def getRandom3DVector(seed, double size):
    ...
    return Vector((
        uniformRandomDouble(_seed + 542655, -size, size),
        uniformRandomDouble(_seed + 765456, -size, size),
        uniformRandomDouble(_seed + 123587, -size, size)
    ))
...
def uniformRandomDoubleWithTwoSeeds(seed1, seed2, double min, double max):
    return uniformRandomDouble((seed1 * 674523 + seed2 * 3465284) % 0x7fffffff, min, max)

cdef void randomNormalized3DVector(int seed, float *vector, float size):
    cdef double a = uniformRandomDouble(seed, 0, 2 * PI)
    cdef double b = uniformRandomDouble(seed + 234452, -1, 1)
...
cdef int uniformRandomInteger(int x, int min, int max):
    return <int>uniformRandomDouble(x, min, <double>max + 0.9999999)
...
cdef float uniformRandomFloat(int x, float min, float max):
    '''Generate a random number between min and max using a seed'''
    x = (x<<13) ^ x
    return ((x * (x * x * 15731 + 789221) + 1376312589) & 0x7fffffff) / <float>2147483648.0 * (max - min) + min

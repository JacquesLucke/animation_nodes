...
class ParticleOutput(bpy.types.Node, AnimationNode):
    bl_idname = "an_ParticleInfoOutput"
    bl_label = "ParticleInfoOutput"

    def create(self):
        self.newOutput("Particle", "Particle", "particle")

        self.newInput("Vector", "Location", "location")
        self.newInput("Euler", "Rotation", "rotation")
        self.newInput("Vector", "Velocity", "velocity", hide = True)
        self.newInput("Euler", "Angular Velocity", "angularVelocity", hide = True)
        self.newInput("Float", "Size", "size", hide = True)
        self.newInput("String", "Alive State", "aliveState", hide = True)
        self.newInput("Boolean", "Is Exist", "isExist", hide = True)
        self.newInput("Boolean", "Is Visible", "isVisible", hide = True)
        self.newInput("Float", "Lifetime", "lifetime", hide = True)
        self.newInput("Float", "Birth Time", "birthTime", hide = True)
        self.newInput("Float", "Die Time", "dieTime", hide = True)
        self.newInput("Vector", "Previous Location", "previousLocation", hide = True)
        self.newInput("Euler", "Previous Rotation", "previousRotation", hide = True)
        self.newInput("Vector", "Previous Velocity", "previousVelocity", hide = True)
        self.newInput("Euler", "Previous Angular Velocity", "previousAngularVelocity", hide = True)
        self.newInput("Float List", "Hair Time", "hairTime", hide = True)
        self.newInput("Float List", "Hair Weight", "hairWeight", hide = True)
        self.newInput("Vector List", "Hair Points", "hairPoints", hide = True)
        self.newInput("Vector List", "Hair Points Local", "hairPointsLocal", hide = True)

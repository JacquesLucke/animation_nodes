...
class IntersectLineLineNode(bpy.types.Node, AnimationNode):
    ...
    def getExecutionCode(self):
        ...
        intersection1  = isLinked["closest1"]
        intersection2  = isLinked["closest2"]
        ...
        if intersection1 : yield "    closest1 = mathutils.Vector((0, 0, 0))"
        if intersection2 : yield "    closest2 = mathutils.Vector((0, 0, 0))"
        if isValid: yield "    isValid = False"
        yield "else:"
        if intersection1 : yield "    closest1 = closestPoints[0]"
        if intersection2 : yield "    closest2 = closestPoints[1]"

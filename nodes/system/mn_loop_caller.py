...
class mn_LoopCallerNode(Node, AnimationNode):
    ...
    def draw_buttons(self, context, layout):
        ...
        col.operator(
            "wm.call_menu", text="New Loop").name = "mn.add_loop_node_menu"
        row = col.row(align=True)
        row.prop(self, "selectedLoop", text="")
        setActive = row.operator(
            "mn.update_active_loop", text="", icon="FILE_REFRESH")
    ...
    def updateSockets(self, socketStartValue=(None, None)):
        ...
        if startNode is None:
            ...
            for socket in fromListSockets:
                ...
                self.inputs.new(
                    idName, socket.customName + " List", socket.identifier + "list")
                self.outputs.new(
                    idName, socket.customName + " List", socket.identifier + "list")

            for socket in fromSingleSockets:
                inputSocket = self.inputs.new(
                    socket.bl_idname, socket.customName, socket.identifier)
